...
class PyHunter:
    ...
    def connect(self, url: str) -> Response | None:
        ...
        try:
            ...
            self.log(
                f"Connection Blocked [{response.status_code}], Rotating User Agents..."
            )
        ...
        for ua in self.USER_AGENTS:
            ...
            try:
                ...
                self.log(f"New UA Failed [{err}]")
                continue

        self.log("Failed to Connect With All Availible User-Agents")
        return None

    def analyse_verification_method(
        self, valid_user: str, invalid_user: str = "PH000XIP12"
    ) -> dict:

        valid_response: Response | None = self.connect(self.url.format(valid_user))
        if valid_response is None:
            return {"Error": "Cannot Connect to Valid User Profile"}

        invalid_response: Response | None = self.connect(self.url.format(invalid_user))
        ...
        if valid_response.status_code != invalid_response.status_code:
            ...
            self.log(
                f"Method Found: Status Code +[{valid_response.status_code}] VS -[{invalid_response.status_code}]"
            )
            method_found = True

        if not method_found and valid_response.url.replace(
            valid_user, "@@@@@"
        ) != invalid_response.url.replace(invalid_user, "@@@@@"):
        ...
        if not method_found and (
            valid_len := len(valid_response.text.replace(valid_user, "@@@@@"))
        ) != (invalid_len := len(invalid_response.text.replace(invalid_user, "@@@@@"))):
            mapping["check-type"] = "length"
            mapping["criteria"] = {"valid": valid_len, "invalid": invalid_len}
            method_found = True

        if not method_found:
            for key_string in ("404", "Error", "Not Found"):
                if (
                    key_string in invalid_response.text
                    and key_string not in valid_response.text
                ):
    ...
    def _test_cookies(self, username: str, mapping: dict) -> bool:
        ...
        try:
            response = tmp_session.get(
                self.url.format(username), headers=self.headers, timeout=10
            )
        ...
        match mapping["check-type"]:
            ...
            case "url":
                if (
                    response.url.replace(username, "@@@@@")
                    != mapping["criteria"]["valid"]
                ):
                    self.log(
                        f"Cookies Required, URL [{response.url.replace(username, '@@@@@')}]"
                    )
                    return True

            case "length":
                current_len: int = len(response.text)
                if abs(current_len - mapping["criteria"]["valid"]) > abs(
                    current_len - mapping["criteria"]["invalid"]
                ):
                    self.log(
                        f"Cookies Required, Len [{current_len} is Closer to InValid Len than Valid Len]"
                    )
                    return True
